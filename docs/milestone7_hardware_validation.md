...
The board-independent implementation is in place and the fresh Vivado 2026.1
synthesis check reports that `arty_m7_camera_ethernet_top` elaborates on
`xc7a100tcsg324-1` with zero errors and zero critical warnings:

| Pre-route synthesis metric | Result |
|---|---:|
| Slice LUTs | 13,557 (21.38%) |
| Slice registers | 28,705 (22.64%) |
| Block RAM tiles | 16 (11.85%) |
| DSPs | 0 (0.00%) |

That checkpoint is synthesis evidence only, not a routed timing result. The
current report and checkpoint are regenerated by `scripts/check_m7_synthesis.tcl`
under the ignored `artifacts/m7_runs/synthesis/` directory.

The current CDC report contains the expected XPM asynchronous-FIFO pointer
findings and the inherited reset/PHY crossings from M5, plus M7's intentional
toggle-and-stable-bus control crossings. It remains a physical-review artifact;
no crossing should be waived silently when routed timing and hardware bring-up
are completed.
